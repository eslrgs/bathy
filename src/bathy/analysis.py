...
def _gradients(data: xr.DataArray) -> tuple[np.ndarray, np.ndarray, float, float]:
    """Return (gy, gx, dy, dx) for gradient computation."""
    dy, dx = _cell_size_metres(data)
    gy, gx = np.gradient(data.values, dy, dx)
    return gy, gx, dy, dx
